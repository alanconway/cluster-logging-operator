...
func TestLokiOutput(t *testing.T) {
	...
	testCase(
		...
		func(t *testing.T) {
			...
			// Check expected Loki labels
			r, err := l.QueryUntil(`{kubernetes_host=~".+"}`, 1)
			assert.NoError(t, err)
			want := map[string]string{
				...
				"kubernetes_host":           f.Pod.Spec.NodeName, // Host should always be included
			}
			labels := r[0].Stream
			// Added by Grafana plugin.
			delete(labels, "fluentd_thread")
			assert.Equal(t, want, labels)
